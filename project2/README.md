...
- We are not sure if we can do that. We believe we can't.

### SegmentRouting
_ With segment routing, we are sure that we can implement the blue path. Using "global segments", the packet will follow the shortest path to the node, and will use ECMP if present. So, we simply have to make sure that C1-C2-C3 and C1-C3 have the same total cost.
- BUT, we believe segment routing is not implemented on Cisco routers.
- However, we can distribute the labels by “hand” on the routers to simulate segment routing. Only 2 paths on a small number of routers, this should not take too long.